 <div class="row">
          <div class="col-lg-12">
            <h1 align="center">Persediaan Alat Tulis Kantor</h1>
            <ol class="breadcrumb">
            </ol>
          </div>
        </div>
...
<?php 
include 'koneksi.php';
$sql= mysql_query("select * from databarang where kode=1");
      ...
      <form id="form" method="post" action="">
        ...
        <div class="form-group">
         <label class="control-label" for="output">Pilih Output :</label>
          ...
          <div class="form-group">
          <a href="views/fullcetak.php?kode=1" class="btn btn-primary" target="_blank" id="c_kes">Pdf</a>  
          <a href="views/excelfull.php?kode=1" class="btn btn-primary" id="c_kes">Excell</a>    
          </div>
          </div>
          </form>
          </div>
...
$update = mysql_query("update databarang set nama_barang='$_POST[nama]', stok_awal='$_POST[stok]', stok_akhir='$_POST[sta]', harga_satuan='$_POST[hsatuan]', satuan='$_POST[satuan]' where no='$_POST[no]'");
if ($update){
...
if ($simpan) {
  ...
  echo mysql_error();
  echo "<script>alert('Barang gagal ditambahkan'); document.location='index.php?page=daftar_barang'</script>";
}
